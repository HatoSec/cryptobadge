...
static const uint8_t font5x7[95][7] = {
    ...
    /* 35 # */     {0x00,0x0A,0x1F,0x0A,0x1F,0x0A,0x00},
    ...
    /* 44 , */     {0x00,0x00,0x00,0x00,0x00,0x0C,0x04},
    ...
    /* 64 @ */     {0x0E,0x11,0x17,0x15,0x15,0x10,0x0E},
    ...
    /*126 ~ */     {0x00,0x0D,0x1B,0x00,0x00,0x00,0x00},
};
...
void gfx_fill_rect(uint16_t *fb, int x, int y, int w, int h, uint16_t color)
{
    for (int j = 0; j < h; j++) {
        int yy = y + j;
        if (yy < 0 || yy >= GFX_HEIGHT) {
            continue;
        }
        for (int i = 0; i < w; i++) {
            int xx = x + i;
            if (xx < 0 || xx >= GFX_WIDTH) {
                continue;
            }
            fb[yy * GFX_WIDTH + xx] = color;
        }
    }
}
...
{
    gfx_line(fb, x, y, x + w - 1, y, color);
    gfx_line(fb, x, y, x, y + h - 1, color);
    gfx_line(fb, x + w - 1, y, x + w - 1, y + h - 1, color);
    gfx_line(fb, x, y + h - 1, x + w - 1, y + h - 1, color);
}
    ...
    while (*str) {
        ...
        for (int row = 0; row < 7; row++) {
            ...
            for (int col = 0; col < 5; col++) {
                int xx = x + col;
                int yy = y + row;
                if (xx < 0 || xx >= GFX_WIDTH || yy < 0 || yy >= GFX_HEIGHT) {
                    continue;
                }
                fb[yy * GFX_WIDTH + xx] = (b & (1 << (4 - col))) ? color : bg;
            }
        }
        x += 6;
        str++;
